...
- Structured `inspectNcs()` API that reports header metadata, instruction offsets, semantic operand parts, jump targets, and ACTION names from the same decoder used by textual disassembly.
- Static `NWScriptCompiler.inspectNcs()` for inspecting bytecode without creating a compiler instance.
